...
class ATSS_YOLOV3Head(AnchorHead):
    ...
    def __init__(self,
        ...
        super(ATSS_YOLOV3Head, self).__init__(
            num_classes,
            in_channels,
            init_cfg=init_cfg)
        # Check params
    ...
    def loss(self,
        ...
        target_maps_list, neg_maps_list = self.get_targets(
            anchor_list, valid_flag_list, gt_bboxes, gt_labels)
    ...
    def get_targets(self,
        ...
        #     anchor_list_cat[i] = torch.cat(anchor_list[i])
        #     valid_flag_list_cat[i] = torch.cat(valid_flag_list[i])
        if gt_bboxes_ignore_list is None:
            gt_bboxes_ignore_list = [None for _ in range(num_imgs)]
        if gt_labels_list is None:
            gt_labels_list = [None for _ in range(num_imgs)]
        results = multi_apply(self._get_targets_single, anchor_list,
                              valid_flag_list, num_level_anchors_list, gt_bboxes_list,
                              gt_labels_list, img_metas, label_channels=label_channels,
    ...
    def _get_target_single(self,
        ...
        flat_anchors_cat = torch.cat(flat_anchors)
        valid_flags_cat = torch.cat(valid_flags)
        inside_flags = anchor_inside_flags(flat_anchors_cat, valid_flags_cat,
                                           img_meta['img_shape'][:2],
                                           self.train_cfg.allowed_border)
        if not inside_flags.any():
            return (None, ) * 7
        # assign gt and sample anchors
        anchors = flat_anchors_cat[inside_flags, :]
        ...
        for i in range(len(flat_anchors)):
            anchor_strides.append(
                torch.tensor(self.featmap_strides[i],
                             device=gt_bboxes.device).repeat(len(flat_anchors[i])))

        target_map = anchors.new_zeros(anchors.size(0), self.num_attrib)
        ...
        if unmap_outputs:
            num_total_anchors = flat_anchors_cat.size(0)

            target_map = unmap(target_map, num_total_anchors, inside_flags)
            neg_map = unmap(neg_map, num_total_anchors, inside_flags)

...
class YOLOV3_ATSS_Head(BaseDenseHead, BBoxTestMixin):
    ...
    def __init__(self,
        ...
        super(YOLOV3_ATSS_Head, self).__init__(init_cfg)
        # Check params
    ...
    def get_targets(self,
        ...
        Args:
            ...
            valid_flag_list (list[list[Tensor]]): Multi level valid
        ...
        flat_anchor_list = anchor_list[:]
        flat_valid_flag_list = valid_flag_list[:]
        for i in range(num_imgs):
            assert len(anchor_list[i]) == len(valid_flag_list[i])
            flat_anchor_list[i] = torch.cat(flat_anchor_list[i])
            flat_valid_flag_list[i] = torch.cat(flat_valid_flag_list[i])
        ...
        for i in range(len(anchor_list)):
            anchor_strides.append(
                torch.tensor(self.featmap_strides[i],
                             device=gt_bboxes.device).repeat(len(anchor_list[i])))

        anchor_strides = torch.cat(anchor_strides)

        concat_anchors = torch.cat(anchor_list)
